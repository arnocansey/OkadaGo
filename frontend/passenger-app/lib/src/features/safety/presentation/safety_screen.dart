...
class _SafetyScreenState extends ConsumerState<SafetyScreen> {
  ...
  Future<void> _showAddContactDialog() async {
    ...
    try {
      ...
      await ref.read(passengerSafetyRepositoryProvider).createContact(
  ...
  Future<void> _showVerifyContactDialog(SafetyContact contact) async {
    ...
    try {
      ...
      await ref.read(passengerSafetyRepositoryProvider).requestContactVerification(contactId: contact.id);
      if (!mounted) return;
      ...
      if (confirm == true) {
        ...
        await ref.read(passengerSafetyRepositoryProvider).verifyContactOtp(
  ...
  Future<void> _triggerSos(String? rideId) async {
    try {
      ...
      await ref.read(passengerSafetyRepositoryProvider).triggerSos(
            description: 'Passenger triggered SOS from mobile app.',
            rideId: rideId,
          );
      setState(() {
        _reloadTick++;
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: FutureBuilder<SafetyOverview>(
        ...
        future: ref.read(passengerSafetyRepositoryProvider).getOverview(),
                  ...
                  onPressed: _submitting
                      ...
                      : () async {
                          try {
                            ...
                            await ref.read(passengerSafetyRepositoryProvider).shareTrip(rideId: data.activeRideId!, start: true);
                                ...
                                IconButton(
                                  ...
                                  onPressed: _submitting
                                      ...
                                      : () async {
                                          try {
                                            setState(() => _submitting = true);
                                            await ref.read(passengerSafetyRepositoryProvider).deleteContact(contact.id);
                                            setState(() {
                                              _reloadTick++;
