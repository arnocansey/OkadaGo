...
function EarningsContent({
  ...
  const currency =
    settlementWallet?.currency ?? completedRides[0]?.currency ?? "GHS";
  ...
  const deficitRecoveryAmount = isDeficitLocked
    ? roundCurrency(
        Math.max(1, deficitAmount - riderDeficitOfflineThreshold + 1),
      )
    : 0;
  ...
  const payoutRequestsQuery = useQuery({
    ...
    queryFn: () =>
      requestJson<RiderPayoutRequestRecord[]>(
  ...
  const settlementPreviewMutation = useMutation({
    mutationFn: async () => {
      ...
      return postJson<SettlementPreviewResponse, unknown>(
        "/wallets/settlement-preview",
        {
          currency: selectedRide.currency,
          paymentMethod: settlementForm.paymentMethod,
          totalFare: parseNumber(
  ...
  const payoutEligibilityMutation = useMutation({
    mutationFn: async () =>
      postJson<PayoutEligibilityResponse, unknown>(
        "/wallets/payout-eligibility",
        {
          availableBalance,
          ...
          hasPendingPayout: (payoutRequestsQuery.data ?? []).some((request) =>
            ["REQUESTED", "REVIEWING", "APPROVED", "PROCESSING"].includes(
              request.status,
            ),
          ),
        },
  ...
  return (
    <main className="exact-rider-content">
      <section className="exact-rider-page-head">
        <div>
          ...
          <p className="body-muted">
            Live settlement balances and completed-trip payouts, based on the
            same backend wallet and ride records.
          </p>
        </div>
      </section>
      ...
      <section className="exact-rider-policy-grid">
        <article className="workbench-card exact-rider-policy-card">
          <div className="workbench-header">
            ...
            <p className="body-muted">
              Settlement deficit rules now control when the rider can stay
          ...
          <div className="exact-rider-policy-list">
            <div className="exact-rider-policy-row">
              ...
              <strong>
                {formatMoney(currency, riderDeficitWarningThreshold)}
              </strong>
              <small>
                Warnings appear and payouts stay restricted while the rider is
                in deficit.
              </small>
            </div>
            <div className="exact-rider-policy-row">
              ...
              <strong>
                {formatMoney(currency, riderDeficitOfflineThreshold)}
              </strong>
              <small>
                The rider is forced offline automatically at this deficit level.
              </small>
            </div>
            <div className="exact-rider-policy-row">
              <span>Back online rule</span>
              <strong>
                Below {formatMoney(currency, riderDeficitOfflineThreshold)}
              </strong>
              <small>
                You only need to pay enough to move below the hard lock
                threshold, not always back to zero.
              </small>
            </div>
          </div>
        </article>
        ...
        <article className="workbench-card exact-rider-policy-card exact-rider-policy-card-state">
          ...
          <div
            className={`exact-rider-policy-state ${isDeficitLocked ? "locked" : isDeficitWarning ? "warning" : hasDeficit ? "watch" : "healthy"}`}
          >
          ...
          <div className="exact-rider-deficit-actions exact-rider-deficit-actions-inline">
            ...
            <div className="exact-rider-inline-actions">
              ...
              >
                {deficitTopUpMutation.isPending
                  ? "Redirecting..."
                  : "Pay deficit"}
              </button>
              {isDeficitLocked ? (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    setDeficitTopUpAmount(String(deficitRecoveryAmount))
                  }
                >
                  Use unlock amount
                </button>
            ...
            {!hasDeficit ? (
              <p className="body-muted">
                No active rider deficit yet. This action becomes available as
                soon as settlement debt appears.
              </p>
            ) : null}
          </div>
        </article>
      ...
      {isDeficitWarning ? (
        <section
          className={`exact-rider-deficit-banner ${isDeficitLocked ? "locked" : "warning"}`}
        >
          <div>
            <p className="workspace-tag">
              {isDeficitLocked
                ? "auto offline threshold reached"
                : "deficit warning"}
            </p>
            <h3>Rider deficit: {formatMoney(currency, deficitAmount)}</h3>
            <p>
              {isDeficitLocked
            ...
            <div className="exact-rider-summary-row compact">
              ...
              <strong>
                {isDeficitLocked
                  ...
                  : formatMoney(
                      currency,
                      Math.max(0, riderDeficitOfflineThreshold - deficitAmount),
                    )}
              </strong>
            </div>
          </div>
      ...
      {topUpStatus === "failed" ? (
        <div className="empty-state empty-state-spaced">
          ...
          <p>
            {topUpMessage ??
              "Paystack could not confirm the rider deficit payment."}
          </p>
        </div>
      ) : null}
        ...
        <div className="exact-rider-earnings-column">
          <article className="workbench-card">
            <div className="workbench-header">
              ...
              <p className="body-muted">
                Each completed ride below contributes to your settlement balance
                and payout history.
              </p>
            </div>
            {completedRides.length === 0 ? (
              <div className="empty-state empty-state-spaced">
                ...
                <p>
                  Once you complete trips, your payout activity will appear here
                  automatically.
                </p>
              </div>
            ) : (
          ...
          <article className="workbench-card">
            <div className="workbench-header">
              ...
              <p className="body-muted">
                Inspect how a completed trip settles into rider earnings using
                your live commission setup.
              </p>
            </div>
            ...
            {completedRides.length === 0 ? (
              <div className="empty-state empty-state-spaced">
                ...
                <p>
                  Settlement previews will unlock once you have at least one
                  completed trip.
                </p>
              </div>
            ) : (
              <>
                <div className="exact-rider-form-grid exact-rider-form-grid-spaced">
                  ...
                  <div className="field-group">
                    <label
                      className="field-label"
                      htmlFor="rider-settlement-payment-method"
                    >
                      Payment method
                    </label>
                    <select
                  ...
                  <div className="field-group">
                    <label
                      className="field-label"
                      htmlFor="rider-settlement-gateway-fee"
                    >
                      Gateway fee
                    </label>
                    <input
                  ...
                  <div className="field-group">
                    <label
                      className="field-label"
                      htmlFor="rider-settlement-rider-bonus"
                    >
                      Rider bonus
                    </label>
                    <input
                  ...
                  <div className="field-group">
                    <label
                      className="field-label"
                      htmlFor="rider-settlement-refund-amount"
                    >
                      Refund amount
                    </label>
                    <input
                ...
                <div className="exact-rider-inline-actions">
                  ...
                  >
                    {settlementPreviewMutation.isPending
                      ? "Previewing..."
                      : "Preview settlement"}
                  </button>
                  {selectedRide ? (
                  ...
                  <div className="exact-rider-finance-grid">
                    ...
                    <article className="workbench-subcard">
                      ...
                      <div className="exact-rider-payout-list compact">
                        {settlementPreviewMutation.data.lineItems.map(
                          (line) => (
                            <div
                              className="exact-rider-payout-row"
                              key={line.label}
                            >
        ...
        <div className="exact-rider-earnings-column">
          <article className="workbench-card">
            <div className="workbench-header">
              ...
              <p className="body-muted">
                The live rider wallet remains the source of truth for what is
            ...
            <div className="exact-rider-summary-stack">
              ...
              <div className="exact-rider-summary-note">
                ...
                <p>
                  Use the settlement preview below to inspect rider net values,
                  then request payout from your live available balance.
                </p>
              </div>
            </div>
          </article>
          ...
          <article className="workbench-card">
            <div className="workbench-header">
              ...
              <p className="body-muted">
                Send a withdrawal request from your available rider settlement
                balance.
              </p>
            </div>
            ...
            <div className="exact-rider-form-grid exact-rider-form-grid-spaced">
              ...
              <div className="field-group">
                <label
                  className="field-label"
                  htmlFor="rider-payout-destination"
                >
            ...
            <div className="exact-rider-inline-actions">
              <button
                ...
                disabled={
                  payoutEligibilityMutation.isPending ||
                  isDeficitWarning ||
                  availableBalance <= 0
                }
              >
              ...
              <p className="body-muted">
                {isDeficitWarning ? (
                  ...
                  <>
                    Minimum payout is{" "}
                    <strong>{formatMoney(currency, 20)}</strong>.
                  </>
                )}
              </p>
            </div>
            ...
            {payoutEligibilityMutation.data ? (
              <div className="exact-rider-eligibility-card">
                ...
                <p>
                  ...
                  {formatMoney(
                    currency,
                    payoutEligibilityMutation.data.requestedAmount,
                  )}{" "}
                  leaves{" "}
                  {formatMoney(
                    currency,
                    payoutEligibilityMutation.data.remainingBalance,
                  )}{" "}
                  available.
                </p>
              </div>
            ...
            {payoutRequestMutation.isSuccess ? (
              <div className="exact-rider-eligibility-card success">
                ...
                <p>
                  ...
                  is now queued for review to{" "}
                  {payoutRequestMutation.data.payoutRequest.destinationLabel}.
                </p>
              </div>
            ) : null}
            ...
            <div className="exact-rider-payout-history">
              ...
              {payoutRequestsQuery.isLoading ? (
                ...
                <div className="empty-state empty-state-spaced">
                  ...
                  <p>
                    Your rider payout requests will appear here once you submit
                    one.
                  </p>
                </div>
              ) : (
                    ...
                    <div className="exact-rider-payout-row" key={request.id}>
                      <div>
                        ...
                        {request.rejectionReason ? (
                          <small>{request.rejectionReason}</small>
                        ) : null}
                      </div>
                      <div className="exact-rider-payout-meta">
                        ...
                        <span
                          className={`exact-rider-status-chip is-${request.status.toLowerCase()}`}
                        >
    ...
    <main className="exact-rider-content">
      <section className="exact-rider-page-head">
        <div>
          ...
          <h1>Trip history</h1>
          <p className="body-muted">
            Separate trip management view inspired by the reference rider
            history state, now wired to live ride records.
          </p>
        </div>
      </section>
      ...
      <section className="workbench-card">
        <div className="workbench-header">
          ...
          <p className="body-muted">
            Every ride assigned to this rider profile, sorted newest first from
            the backend.
          </p>
        </div>
        {riderRides.length === 0 ? (
          <div className="empty-state empty-state-spaced">
            ...
            <p>
              Trip history will populate here as soon as dispatch starts
              assigning rides to this rider.
            </p>
          </div>
        ) : (
          <div className="exact-rider-records">
...
export function RiderPortalPage({
  ...
  const rider = useMemo(
    () =>
      (ridersQuery.data ?? []).find((entry) => entry.id === riderProfileId) ??
      null,
  ...
  const activeRide =
    riderRides.find((ride) =>
      ["assigned", "arriving", "arrived", "started"].includes(ride.status),
    ) ?? null;
  ...
  const completionRate =
    riderRides.length === 0
      ? 0
      : Math.round((completedCount / riderRides.length) * 100);

  const settlementWallet =
    (walletsQuery.data ?? []).find(
      (wallet) => wallet.type === "RIDER_SETTLEMENT",
    ) ??
    walletsQuery.data?.[0] ??
    null;
  const currency =
    settlementWallet?.currency ?? completedRides[0]?.currency ?? "GHS";
  const settlementAvailableBalance = parseNumber(
    settlementWallet?.availableBalance,
  );
  const deficitAmount =
    settlementAvailableBalance < 0 ? Math.abs(settlementAvailableBalance) : 0;
      ...
      ? [
          {
            id: rider.id,
            position: [
              parseNumber(rider.currentLatitude),
              parseNumber(rider.currentLongitude),
            ] as [number, number],
  ...
  const dashboardMapCenter =
    currentMapPosition ?? ([5.6037, -0.187] as [number, number]);
  ...
  const updateAvailability = useMutation({
    ...
    onMutate: async (onlineStatus) => {
      ...
      const previousRiders = queryClient.getQueryData<RiderRecord[]>([
        "riders",
      ]);
  ...
  return (
    <ImmersivePage className="exact-rider-page">
      <div className="exact-rider-statebar">
        {displayIsOnline
          ? "LIVE RIDER DASHBOARD - ONLINE"
          : "LIVE RIDER DASHBOARD - OFFLINE"}
      </div>
        ...
        <header className="exact-rider-topnav">
          <div className="exact-rider-topnav-left">
            ...
            <nav className="exact-rider-navlinks">
              <a
                href="/rider"
                className={screen === "dashboard" ? "active" : undefined}
              >
                Dashboard
              </a>
              <a
                href="/rider/earnings"
                className={screen === "earnings" ? "active" : undefined}
              >
                Earnings
              </a>
              <a
                href="/rider/trips"
                className={screen === "trips" ? "active" : undefined}
              >
                Trips
              </a>
            </nav>
        ...
        <div className="exact-rider-body">
          <aside className="exact-rider-sidebar">
            ...
            <section className="exact-rider-hero-card">
              ...
              <div className="exact-rider-bonus-card">
                <span>
                  {rider?.serviceZone?.name ?? "No service zone assigned"}
                </span>
                <p>
                  {rider?.city ??
                    "Update your rider profile city to improve dispatch matching."}
                </p>
              </div>
            </section>
                  ...
                  <div className="exact-stars">
                    {[0, 1, 2, 3, 4].map((item) => (
                      <Star
                        ...
                        size={12}
                        className={
                          item < Math.max(1, Math.round(completionRate / 25))
                            ? "filled"
                            : ""
                        }
                      />
            ...
            <div className="exact-rider-module-stack">
              {isDeficitWarning ? (
                <section
                  className={`exact-rider-finance-alert ${isDeficitLocked ? "locked" : "warning"}`}
                >
                  <strong>
                    {isDeficitLocked
              ...
              <section className="workbench-card" id="rides">
                <div className="workbench-header">
                  ...
                  <p className="body-muted">
                    Progress your currently assigned ride with real backend
                    lifecycle updates.
                  </p>
                </div>
                {!activeRide ? (
                  <div className="empty-state empty-state-spaced">
                    ...
                    <p>
                      Go online and wait for a dispatch assignment to appear
                      here.
                    </p>
                  </div>
                ) : (
                  <div className="exact-rider-live-ride-details">
                    ...
                    <p className="body-muted">
                      Destination:{" "}
                      <strong>{activeRide.destinationAddress}</strong>
                    </p>
                    <p className="body-muted">
                      Fare:{" "}
                      <strong>
                    ...
                    {nextActionLabel && nextActionStatus ? (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          advanceRideStatus.mutate(nextActionStatus)
                        }
                        disabled={advanceRideStatus.isPending}
                      >
                        {advanceRideStatus.isPending
                          ? "Updating..."
                          : nextActionLabel}
                      </button>
                    ) : null}
          ...
          {screen === "dashboard" ? (
            <section className="exact-rider-map">
              ...
              <div className="exact-rider-badge">
                {activeRide
                  ? `${formatStatus(activeRide.status)} ride active`
                  : "No active trips"}
              </div>
            </section>
          ) : null}
          ...
          {screen === "earnings" ? (
            <EarningsContent
              ...
              riderCommissionPercent={
                parseNumber(rider?.commissionPercent) || 12
              }
              token={session.token}
          ...
          {screen === "trips" ? (
            <TripsContent riderRides={riderRides} activeRide={activeRide} />
          ) : null}
        </div>
      </div>
    </ImmersivePage>
