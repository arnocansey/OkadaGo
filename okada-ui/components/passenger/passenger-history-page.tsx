...
export function PassengerHistoryPage() {
  ...
  const completedCount = passengerRides.filter(
    (ride) => ride.status === "completed",
  ).length;
  const cancelledCount = passengerRides.filter(
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="body-muted mt-2 max-w-2xl">
              View your real completed and cancelled trips from the backend ride
              ledger.
            </p>
          </div>
        ...
        {filteredRides.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-8 text-center shadow-sm">
            ...
            <p className="mt-2 text-slate-500">
              Once you complete or cancel a trip, it will appear in this history
