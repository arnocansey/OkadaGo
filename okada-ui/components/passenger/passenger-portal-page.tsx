...
function isWithinGhanaBounds(latitude: number, longitude: number) {
  return (
    latitude >= 4.4 && latitude <= 11.3 && longitude >= -3.4 && longitude <= 1.4
  );
}
...
function queryMatchesResolved(
  ...
  const candidateValues = [place.label, place.displayName ?? ""].map((value) =>
...
export function PassengerPortalPage() {
  ...
  const [rideType, setRideType] = useState<"standard_bike" | "express_bike">(
    "standard_bike",
  );
  const [paymentMethod, setPaymentMethod] = useState<
    "wallet" | "cash" | "card" | "mobile_money"
  >("wallet");
  ...
  const [pickupLocationError, setPickupLocationError] = useState<string | null>(
    null,
  );
  ...
  const [pickupSuggestionsPending, setPickupSuggestionsPending] =
    useState(false);
  const [pickupSuggestionsError, setPickupSuggestionsError] = useState<
    string | null
  >(null);
  const [resolvedPickup, setResolvedPickup] =
    useState<ForwardGeocodeResponse | null>(null);
  const [destinationSessionToken] = useState(() => createPlaceSearchSession());
  const [destinationSuggestions, setDestinationSuggestions] = useState<
    PlaceSuggestion[]
  >([]);
  const [destinationSuggestionsPending, setDestinationSuggestionsPending] =
    useState(false);
  const [destinationSuggestionsError, setDestinationSuggestionsError] =
  ...
  useEffect(() => {
    ...
    if (
      ...
      queryMatchesResolved(pickupAddress, resolvedPickup)
    ) {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          if (
            !lookup.displayName ||
            lookup.label.trim().toLowerCase() === "current location"
          ) {
            throw new Error(
              "Current location could not be resolved to a real place yet.",
            );
          }
        ...
        setForm((current) => {
          const destinationLatitude = tryParseCoordinate(
            current.destinationLatitude,
          );
          const destinationLongitude = tryParseCoordinate(
  ...
  const selectedZone = useMemo(
    () =>
      (zonesQuery.data ?? []).find((zone) => zone.id === form.serviceZoneId) ??
      null,
    [form.serviceZoneId, zonesQuery.data],
  );
  ...
  const pickupLongitude =
    tryParseCoordinate(form.pickupLongitude) ??
    resolvedPickup?.longitude ??
    null;
  const destinationLatitude =
    tryParseCoordinate(form.destinationLatitude) ??
    resolvedDestination?.latitude ??
    null;
  const destinationLongitude =
    tryParseCoordinate(form.destinationLongitude) ??
    resolvedDestination?.longitude ??
    null;
  const pickupAddress =
    form.pickupAddress.trim() || resolvedPickup?.label || "";
  const destinationAddress =
    form.destinationAddress.trim() || resolvedDestination?.label || "";
  ...
  const activeRide = passengerRides.find((ride) =>
    ["searching", "assigned", "arriving", "arrived", "started"].includes(
      ride.status,
    ),
  );
  ...
  const onlineRiderCount = (ridersQuery.data ?? []).filter(
    (rider) => rider.onlineStatus,
  ).length;
  ...
  const mapZoom =
    pickupLatitude != null || destinationLatitude != null ? 13 : 12;
  ...
  const estimateMutation = useMutation({
    mutationFn: async () => {
      ...
      if (
        ...
        throw new Error(
          "Set your pickup and destination so the route can be mapped first.",
        );
      }
  ...
  const createRideMutation = useMutation({
    mutationFn: async () => {
      if (!passengerProfileId) {
        throw new Error(
          "Your passenger profile is missing from the current session. Please sign out and sign back in.",
        );
      }

      if (!selectedZone) {
        throw new Error(
          "No active service zone is configured yet. Set up a Ghana service zone before requesting rides.",
        );
      }
      ...
      return postJson<RideCreationResponse, unknown>("/rides/request", {
        ...
        estimatedDistanceKm:
          routePreviewQuery.data?.distanceKm ??
          Number(form.estimatedDistanceKm),
        estimatedDurationMinutes:
          routePreviewQuery.data?.durationMinutes ??
  ...
  const createDeliveryMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!selectedZone) {
        throw new Error(
          "No active service zone is configured yet. Set up a Ghana service zone before requesting deliveries.",
        );
      }
      ...
      if (
        ...
        throw new Error(
          "Set your pickup and dropoff so the delivery route can be mapped first.",
        );
      }
      ...
      if (
        ...
        throw new Error(
          "Add recipient and package details before requesting delivery.",
        );
      }

      return postJson<DeliveryCreationResponse, unknown>(
        "/deliveries/request",
        {
          passengerProfileId,
          serviceZoneId: selectedZone.id,
        ...
        <aside className="exact-passenger-sidebar">
            ...
            <section className="exact-sidebar-block">
              ...
              <div className="button-row" style={{ marginTop: 18 }}>
                {(["ride", "delivery"] as const).map((mode) => (
                  <button
                    key={mode}
                    className={
                      bookingMode === mode ? "button" : "button-secondary"
                    }
                    type="button"
              ...
              <details className="exact-dev-disclosure">
                ...
                <p className="body-muted">
                  The service zone selector and raw route fields are only for
                  manual testing while map search and route calculation are
              ...
              {bookingMode === "ride" ? (
                <div className="exact-ride-options">
                  <h3>Choose a ride</h3>
                  {(["standard_bike", "express_bike"] as const).map(
                    (option) => {
          ...
          <footer className="exact-passenger-footer">
            <div className="exact-payment-row">
              <div className="exact-cash-badge">
            ...
            <button
              ...
              disabled={
                createRideMutation.isPending || createDeliveryMutation.isPending
              }
            >
          ...
          <div className="map-shell exact-passenger-map-stage">
            ...
            !liveLocationReady ? (
              <div className="map-empty-note" aria-live="polite">
                <div className="map-empty-note-card">
                  ...
                  <p>
                    Choose a pickup and destination to preview the route on the
                    live passenger map.
                  </p>
                </div>
              </div>
            ...
            <div className="exact-passenger-map-topbar">
              ...
              <div className="exact-passenger-map-chip">
                ...
                <strong>
                  {onlineRiderCount > 0
                    ? `${onlineRiderCount} riders online`
                    : "Matching live"}
                </strong>
              </div>
            </div>
            ...
            <div className="exact-passenger-map-floating exact-passenger-map-floating-left">
              <div className="exact-passenger-map-card route">
                <span className="exact-passenger-map-card-label">
                  Live route preview
                </span>
                <strong>
                  {pickupAddress || "Choose pickup"} to{" "}
