...
import { Bike, Menu, MessageCircle, Phone, Share2, Star } from 'lucide-react';
import { MobileScreenShell } from '../_shared/mobile-screen-shell';
import { MobileStatusBar } from '../_shared/mobile-status-bar';

export default function Tracking() {
  return (
    <MobileScreenShell className="bg-gray-900 flex flex-col">
      <MobileStatusBar className="absolute top-0 text-gray-900" />
      ...
      <div className="absolute inset-0 bg-[#1e293b]">
        ...
        <div className="absolute top-[485px] left-[165px]">
          <div className="w-8 h-8 bg-[#0D6B4A]/30 rounded-full flex items-center justify-center animate-ping" />
          <div className="w-4 h-4 bg-[#0D6B4A] border-2 border-white rounded-full absolute top-2 left-2 shadow-lg" />
        </div>
      ...
      <div className="absolute top-12 left-4 right-4 z-20 flex items-start justify-between">
        <button className="w-12 h-12 bg-white rounded-full shadow-lg flex items-center justify-center text-gray-900 hover:bg-gray-100 active:scale-[0.98] transition-all">
          <Menu size={24} />
        </button>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.15)] z-30 pt-4 pb-[max(2rem,env(safe-area-inset-bottom))] px-5 flex flex-col h-[min(350px,48dvh)]">
        ...
        <div className="flex gap-3 mb-6">
          <button className="flex-1 h-12 rounded-full bg-gray-100 hover:bg-gray-200 active:scale-[0.98] transition-all flex items-center justify-center gap-2 text-gray-900 font-medium text-[14px]">
            <Phone size={18} /> Call
          </button>
          <button className="flex-1 h-12 rounded-full bg-green-50 hover:bg-green-100 active:scale-[0.98] transition-all flex items-center justify-center gap-2 text-[#0D6B4A] font-medium text-[14px]">
            <MessageCircle size={18} /> Chat
          </button>
          <button className="w-12 h-12 rounded-full bg-gray-100 hover:bg-gray-200 active:scale-[0.98] transition-all flex items-center justify-center text-gray-900">
            <Share2 size={18} />
          </button>
        </div>

        <div className="mt-auto flex justify-center">
          <button className="text-red-500 font-medium text-[14px] px-4 py-2 rounded-full hover:bg-red-50 active:scale-[0.98] transition-all">
            Cancel ride
          </button>
        </div>
      </div>
